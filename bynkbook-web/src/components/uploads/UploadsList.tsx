  ...
  return (
    <div className="rounded-lg border border-bb-border bg-bb-surface-card overflow-hidden">
      <div className="px-3 py-2 border-b border-bb-border flex items-center justify-between gap-2">
        ...
        {loading && items.length > 0 ? <div className="text-[11px] text-bb-text-muted">Updating…</div> : null}
      </div>

      {error ? (
        <div className="p-3 text-xs text-bb-status-danger-fg">{error}</div>
      ) : items.length === 0 && loading ? (
        <div className="p-3 space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="space-y-1">
              <div className="h-3 w-48 rounded bg-bb-surface-elevated animate-pulse" />
              <div className="h-3 w-64 rounded bg-bb-surface-elevated animate-pulse" />
            </div>
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="p-3 text-xs text-bb-text-muted">No uploads yet.</div>
      ) : (
            ...
            return (
              <div key={u.id} className="px-3 py-2 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-xs font-semibold text-bb-text truncate">{u.original_filename}</div>
                  ...
                  {importSummary ? <div className="mt-0.5 text-[11px] text-bb-text-muted truncate">{importSummary}</div> : null}
                </div>
